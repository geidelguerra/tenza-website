  ...
  <div>
    ...
    <div v-if="showDebugStats" class="absolute top-2 right-2 z-40 font-mono">
      <div v-if="cameraStats" class="flex justify-end flex-col text-right">
        <pre>
          <span class="font-bold">       Distance:</span> {{ cameraStats.distance.toFixed(2) }}
          <span class="font-bold">    Polar Angle:</span> {{ cameraStats.polarAngle.toFixed(2) }}
          <span class="font-bold">Azimuthal Angle:</span> {{ cameraStats.azimuthalAngle.toFixed(2) }}
        </pre>
      </div>
    </div>
  </div>
